...
pprelordudp <- read.csv("evaluation/simple_streaming.csv", sep=",", as.is=c(numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric))

pprelordudp$avg <- rowMeans(pprelordudp[,2:11])
pprelordudp$sdev <- apply(pprelordudp[,2:11], 1, sd)
pprelordudp$proto <- '0 Workers'

ppdf <- rbind(pprelordudp)
ppdf$upper <- ppdf$avg + ppdf$sdev
ppdf$lower <- ppdf$avg - ppdf$sdev


pp_plot <- ggplot(ppdf, aes(x=num_serializing_workers, y=avg, color=proto)) +
  geom_line() + # size=0.8) +
  ...
  labs(x="worker num", y="throughput [msg/s]")

tikz(file="figs/simple_streaming.tikz", sanitize=TRUE, width=3.4, height=2.3)
pp_plot
dev.off()

ggsave("figs/simple_streaming.pdf", plot=pp_plot, width=3.4, height=2.3)
