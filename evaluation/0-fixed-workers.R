...
ppconserwork <- read.csv("evaluation/0-serializing-workers.csv", sep=",", as.is=c(numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric))
...
ppcondeserwork <- read.csv("evaluation/0-deserializing-workers.csv", sep=",", as.is=c(numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric))
...
pp_plot <- ggplot(ppdf, aes(x=num_workers, y=avg, color=proto)) +
  ...
  ggtitle("Zero fixed workers") +
  labs(x="workers", y="throughput [msg/s]")

tikz(file="figs/0-fixed-workers.tikz", sanitize=TRUE, width=3.4, height=2.3)
pp_plot
dev.off()

ggsave("figs/0-fixed-workers.pdf", plot=pp_plot, width=3.4, height=2.3)
