...
pingpong_net_proxy$proto <- '2 net - actor-proxy'
...
ppdf <- rbin(ppdf, pingpong_net_proxy, pingpong_io)
...
pp_plot <- ggplot(ppdf, aes(x=num_pings, y=avg, color=proto)) +
  ...
  ggtitle("Pingpong multiple remote nodes") +
  labs(x="remote nodes [#]", y="throughput [pongs/s]")

tikz(file="figs/pingpong-multiple.tikz", sanitize=TRUE, width=3.4, height=2.3)
pp_plot
dev.off()

ggsave("figs/pingpong-multiple.pdf", plot=pp_plot, width=5, height=3)
