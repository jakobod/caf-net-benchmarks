...
pingpong_net_master <- read.csv("evaluation/data/pingpong-multiple-10-pings-alloc-net.out", sep=",", as.is=c(numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric))
...
pingpong_io <- read.csv("evaluation/data/pingpong-multiple-10-pings-alloc-io.out", sep=",", as.is=c(numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric, numeric))
...
pp_plot <- ggplot(ppdf, aes(x=num_pings, y=avg, color=proto)) +
  ...
  scale_y_continuous(labels = human_numbers, limits=c(0,4000000), breaks=seq(0, 4000000, 250000)) + # expand=c(0, 0), limits=c(0, 10)
  theme_bw() +
  theme(
...
tikz(file="figs/pingpong-multiple-10-alloc.tikz", sanitize=TRUE, width=3.4, height=2.3)
pp_plot
dev.off()

ggsave("figs/pingpong-multiple-10-alloc.pdf", plot=pp_plot, width=8, height=5)
