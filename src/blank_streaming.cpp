...
behavior source_actor(stateful_actor<source_state>* self, actor sink) {
  ...
  self->state.message.resize(1024);
  std::iota(self->state.message.begin(), self->state.message.end(), 1);
  self->send(sink, hello_atom_v, self);
  return {[=](start_atom) {
            for (size_t i = 0; i < 2; ++i)
              self->send(sink, self->state.message);
...
behavior sink_actor(stateful_actor<sink_state>* self, size_t iterations,
  ...
  return {
    ...
    [=](std::vector<uint64_t>) { ++self->state.count; },
  };
}
...
void net_run_source(net::stream_socket sock, size_t id) {
  ...
  scoped_actor self{sys};
  auto source = sys.spawn(source_actor, *sink);
  self->send(source, start_atom_v);
}
