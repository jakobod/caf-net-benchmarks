...
for pings in 1 10 100; do
  echo "------------------------ ${pings} pings ------------------------"
  net_udp_file="${output_folder}/pingpong-net-udp-${pings}-pings"
  init_file ${net_udp_file}
  for num_nodes in {1..64..1}; do
    printf "${num_nodes}, " >> ${net_udp_file}
    while : ; do
      echo "starting netBench-${net_udp_file} nodes"
      ./release/pingpong_udp -mnetBench -n${num_nodes} -p${pings} -i${duration} >> ${net_udp_file}.out 2> ${net_udp_file}.err
      [[ $? != 0 ]] || break # if program exited with error rerun it.
    done;
  done;
done;
